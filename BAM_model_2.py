...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        if not self.cls_mode:
            ...
            for s in reversed(range(self.num_stages - 1)):
                ...
                self.dec_seg.add(module=dec, name=f"dec{s}")
        
        if not self.cls_mode:
            dec_drop_path = [
                x.item() for x in torch.linspace(0, drop_path, sum(dec_depths))
            ]
            self.dec_edge = PointSequential()
            dec_channels = list(dec_channels) + [enc_channels[-1]]
            for s in reversed(range(self.num_stages - 1)):
                dec_drop_path_ = dec_drop_path[
                    sum(dec_depths[:s]) : sum(dec_depths[: s + 1])
                ...
                self.dec_edge.add(module=dec, name=f"dec{s}")
    ...
    def forward(self, data_dict):
        ...
        point_seg = self.enc(point)
        point_edge = point_seg.copy()
        seg_point = self.dec_seg(point_seg)
        edge_point = self.dec_edge(point_edge)
        return seg_point, edge_point
...
class BAM_PT(nn.Module):
    ...
    def forward(self, pts, gts, gmatrix, idxs):
        ...
        # Point Transformer V3 as DST
        B = pts.shape[0]
        seg_PTV3, edge_PTV3 = self.backbone(data_dict)
        seg_feat = seg_PTV3['feat'].view(B, -1, self.cls_num).transpose(1, 2).contiguous()
        edge_feat = edge_PTV3(data_dict)['feat'].view(B, -1, 2).transpose(1, 2).contiguous()
